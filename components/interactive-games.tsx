...
const treasureHuntTargets = [
  { name: "Eiffel Tower", country: "France", hint: "Iron lattice tower in the City of Light", points: 100 },
  { name: "Great Wall", country: "China", hint: "Ancient fortification stretching thousands of miles", points: 150 },
  { name: "Statue of Liberty", country: "USA", hint: "Gift from France, symbol of freedom", points: 100 },
  { name: "Machu Picchu", country: "Peru", hint: "Ancient Incan citadel in the mountains", points: 200 },
  { name: "Taj Mahal", country: "India", hint: "White marble mausoleum, symbol of love", points: 150 },
]
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {!gameState.isPlaying ? (
          ...
          <Button variant="outline" onClick={() => setGameState((prev) => ({ ...prev, isPlaying: false }))}>
            Stop Game
          </Button>
        )}
      </div>

      {gameState.isPlaying && gameState.currentTarget < treasureHuntTargets.length ? (
        <Card className="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800">
          <CardContent className="pt-6">
            <div className="text-center space-y-3">
              <h4 className="text-lg font-semibold">Find This Landmark!</h4>
              <p className="text-gray-600 dark:text-gray-400">{treasureHuntTargets[gameState.currentTarget]?.hint}</p>
              <Badge variant="secondary">{treasureHuntTargets[gameState.currentTarget]?.points} points</Badge>
          ...
          <div className="space-y-2">
            {culturalMatches.map((item, index) => (
              <Button
                ...
                onClick={() => setGameState((prev) => ({ ...prev, matches: prev.matches + 1 }))}
              >
                {item.item} ({item.type})
              </Button>
            ))}
      ...
      <CardContent>
        ...
        <div className="space-y-4">
          ...
          {activeGame === "riverTrace" && (
            <div className="text-center py-8 text-gray-500">
              River tracing game coming soon! Draw river paths on interactive maps.
            </div>
          )}
        </div>
      </CardContent>
