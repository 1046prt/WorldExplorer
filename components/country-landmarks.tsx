  ...
  return (
    <Card className="p-6 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Famous Landmarks</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {country.landmarks.map((landmark) => (
          <LandmarkModal
            key={landmark.slug}
            landmark={{ ...landmark, country: country.name, countryCode: country.iso2 }}
          >
            <Card className="overflow-hidden hover:shadow-lg transition-all duration-200 cursor-pointer hover:scale-105">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-gray-900 dark:text-white">{landmark.name}</h3>
                  <Badge variant="outline" className="text-xs">
                    {landmark.city}
                  </Badge>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-3 line-clamp-2">{landmark.whyFamous}</p>
