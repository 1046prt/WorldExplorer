...
const natureData = {
  ...
  volcanoes: [
    { name: "Mount Vesuvius", country: "Italy", lastEruption: "1944", type: "Stratovolcano", status: "Active" },
    { name: "Krakatoa", country: "Indonesia", lastEruption: "2018", type: "Caldera", status: "Active" },
    { name: "Yellowstone", country: "USA", lastEruption: "70,000 years ago", type: "Supervolcano", status: "Active" },
  ],
  migrations: [
    { species: "Arctic Tern", route: "Arctic to Antarctic", distance: "71,000 km", season: "Annual" },
    { species: "Monarch Butterfly", route: "Canada to Mexico", distance: "4,800 km", season: "Fall/Spring" },
    { species: "Wildebeest", route: "Serengeti Circuit", distance: "1,800 km", season: "Year-round" },
  ],
}
                ...
                <Card key={index} className="border-green-200 bg-green-50 dark:bg-green-950">
                  <CardHeader className="pb-3">
                    <CardTitle className="flex items-center gap-2 text-green-800 dark:text-green-200">
                      {wonder.type === "Mountain" && <Mountain className="h-5 w-5" />}
                      {wonder.type === "Reef" && <Waves className="h-5 w-5" />}
                      {wonder.type === "Forest" && <TreePine className="h-5 w-5" />}
                      {wonder.name}
                    </CardTitle>
                  ...
                  <CardContent>
                    <div className="space-y-2 text-sm">
                      ...
                      <p>
                        <strong>{wonder.height ? "Height" : "Area"}:</strong> {wonder.height || wonder.area}
                      </p>
                      <p>
          ...
          <TabsContent value="volcanoes" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {natureData.volcanoes.map((volcano, index) => (
                <Card key={index} className="border-red-200 bg-red-50 dark:bg-red-950">
                  <CardHeader className="pb-3">
                    <CardTitle className="flex items-center gap-2 text-red-800 dark:text-red-200">
                      <Zap className="h-5 w-5" />
                      {volcano.name}
                    </CardTitle>
                    <Badge variant={volcano.status === "Active" ? "destructive" : "secondary"}>{volcano.status}</Badge>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2 text-sm">
                      <p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {natureData.migrations.map((migration, index) => (
                <Card key={index} className="border-blue-200 bg-blue-50 dark:bg-blue-950">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-blue-800 dark:text-blue-200">{migration.species}</CardTitle>
